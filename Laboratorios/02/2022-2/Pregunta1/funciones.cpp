...
void insertarAlFinal(struct Lista & tad, int elemento, int sucursal) {

    struct Nodo * ultimoNodo = obtenerUltimoNodo(tad);
    struct Nodo * nuevoNodo = crearNodo(elemento, sucursal, nullptr);
...
struct Nodo * crearNodo(int elemento, int sucursal, struct Nodo * siguiente) {
    struct Nodo * nuevoNodo = new struct Nodo;
    nuevoNodo->elemento = elemento;
...
void insertarEnOrden(struct Lista & tad, int elemento, int sucursal) {
...
void imprime(const struct Lista & tad) {
    ...
    if (esListaVacia(tad)) {
        ...
        while (recorrido != nullptr) {
            ...
            cout << recorrido->elemento << "(" << recorrido->sucursal << ")";
            recorrido = recorrido->siguiente;
        }
        cout << "]" << endl;
    }
...
void fusionListas(Lista &lunes, Lista &martes, Lista &miercoles, Lista &jueves, 
    ...
    int menorHora = 25;
    while(true){
        if(lunes.cabeza->elemento < menorHora){
            menorHora = lunes.cabeza->elemento;
        }
        break;
    }
}